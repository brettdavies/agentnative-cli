...
The linter's job is to take a CLI and tell its operator, human or agent, what fails the standard, why, and what to fix.
Both halves of that sentence shape this channel's voice: a verdict that lands, and a remediation path the reader can act
on without re-reading.
...
high-leverage surface is not yet linted: clap `about=` / `help=` / `long_about=` literals, `eprintln!` / `anyhow::bail!`
messages, panic strings, and audit `label()` returns. The deferred U6 unit of
