...
- **AI agents invoking `anc inspect --output json`** as a step in a larger pipeline. They consume the structured envelope,
...
- **Second-person imperative IS the register.** "Run `anc inspect`", "Set `--audit-profile human-tui`", "Pipe the output
...
- **Present tense for state, imperative for actions.** "The binary is unreadable" + "make the file readable, then re-run
  `anc inspect`." Not "The binary was unreadable" or "You would need to make the file readable."
...
- **Mixing structured output and diagnostic prose on the same stream.** `anc inspect --output json` writes JSON to stdout;
