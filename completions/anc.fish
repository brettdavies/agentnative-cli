...
complete -c anc -n "__fish_anc_needs_command" -f -a "schema" -d 'Print the JSON output schema for `anc check --output json`'
...
complete -c anc -n "__fish_anc_using_subcommand schema" -l output -d 'Output format for the schema document itself' -r -f -a "text\t''
json\t''"
...
complete -c anc -n "__fish_anc_using_subcommand help; and not __fish_seen_subcommand_from check completions generate skill schema help" -f -a "schema" -d 'Print the JSON output schema for `anc check --output json`'
