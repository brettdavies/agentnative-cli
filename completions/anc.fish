...
complete -c anc -n "__fish_anc_needs_command" -l raw -d 'Strip section headers, evidence lines, summary line, and badge hint — emit only `id<TAB>status` per check. Pipe-safe for grep, awk, and downstream tooling that wants the raw verdict stream without prose. Ignored in `--output json` mode'
...
complete -c anc -n "__fish_anc_using_subcommand audit" -l command -d 'Resolve a command from PATH and run behavioral checks against it' -r -f -a "(__fish_complete_command)"
complete -c anc -n "__fish_anc_using_subcommand audit" -l principle -d 'Filter checks by principle number (1-8)' -r
complete -c anc -n "__fish_anc_using_subcommand audit" -l output -d 'Output format' -r -f -a "text\t''
json\t''"
complete -c anc -n "__fish_anc_using_subcommand audit" -l audit-profile -d 'Exemption category for the target. Suppresses checks that do not apply to this class of tool — e.g., TUI apps legitimately intercept the TTY, so `--audit-profile human-tui` skips the interactive-prompt MUSTs. Suppressed checks emit `Skip` with structured evidence so readers see what was excluded' -r -f -a "human-tui\t'TUI-by-design tools (lazygit, k9s, btop). Suppresses interactive-prompt MUSTs and SIGPIPE — their contract is the TTY'
...
always\t''
never\t''"
complete -c anc -n "__fish_anc_using_subcommand audit" -l binary -d 'Run only behavioral checks (skip source analysis)'
complete -c anc -n "__fish_anc_using_subcommand audit" -l source -d 'Run only source checks (skip behavioral)'
...
complete -c anc -n "__fish_anc_using_subcommand audit" -l raw -d 'Strip section headers, evidence lines, summary line, and badge hint — emit only `id<TAB>status` per check. Pipe-safe for grep, awk, and downstream tooling that wants the raw verdict stream without prose. Ignored in `--output json` mode'
...
complete -c anc -n "__fish_anc_using_subcommand completions" -l raw -d 'Strip section headers, evidence lines, summary line, and badge hint — emit only `id<TAB>status` per check. Pipe-safe for grep, awk, and downstream tooling that wants the raw verdict stream without prose. Ignored in `--output json` mode'
...
complete -c anc -n "__fish_anc_using_subcommand emit; and not __fish_seen_subcommand_from coverage-matrix schema help" -f -a "coverage-matrix" -d 'Render the spec coverage matrix (registry → checks → artifact)'
...
complete -c anc -n "__fish_anc_using_subcommand emit; and __fish_seen_subcommand_from help" -f -a "coverage-matrix" -d 'Render the spec coverage matrix (registry → checks → artifact)'
...
complete -c anc -n "__fish_anc_using_subcommand help; and __fish_seen_subcommand_from emit" -f -a "coverage-matrix" -d 'Render the spec coverage matrix (registry → checks → artifact)'
