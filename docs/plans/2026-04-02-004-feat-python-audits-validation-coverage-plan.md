---
title: "feat: Python starter audits, fixture test coverage, and real-world validation"
type: feat
status: done
date: 2026-04-02
...
# feat: Python starter audits, fixture test coverage, and real-world validation

## Overview

Three confidence gaps before GA: (1) Python source audits are empty — shipping 2-3 starter audits proves the
...
The design doc commits to "5-6 key Python audits" in v0.1. Zero shipped. Rather than the full set, we'll ship 2-3
starter audits that prove the ast-grep + tree-sitter-python pipeline works end-to-end, deferring the rest to v0.2. The
...
- R1. At least 2-3 Python source audits implemented and tested
...
- R5. Design doc updated to reflect revised Python audit scope for v0.1

## Scope Boundaries

- Python audits target Click/Typer CLI patterns (the most common Python CLI frameworks)
- Full Python audit set (5-6 audits from design doc) deferred to v0.2
...
- `src/audits/source/python/mod.rs` — empty stub, returns `vec![]`
- `src/audits/source/rust/unwrap.rs` — canonical pattern for implementing a source audit
- `src/audits/source/rust/no_color.rs` — pattern for conditional/trigger-based source audits
...
- No existing solutions for Python ast-grep audits — this is greenfield
...
- **Which 2-3 Python audits to implement**: Focus on audits that (a) prove the pipeline, (b) are meaningful for
...
- **Fixture tests — what's blocking them**: The `#[ignore]` comments say "requires parseable Rust source" or "requires
  ...
  un-ignoring.
- **Real-world validation approach**: Run `agentnative audit <path>` against bird, xurl-rs (local repos), and `ripgrep`
...
- **Do we need a Python fixture project?**: Not for v0.1 starter audits. The Rust fixtures test the audit
  infrastructure. Python audits get unit tests with inline source strings.
...
- [x] **Unit 1: Implement `code-bare-except` Python source audit**
...
- Create: `src/audits/source/python/bare_except.rs`
- Modify: `src/audits/source/python/mod.rs` — register the audit
- Test: inline `#[cfg(test)]` module in `bare_except.rs`

**Approach:**

- Follow the exact pattern from `src/audits/source/rust/unwrap.rs`:
- Unit struct `BareExceptAudit`
- Implement `Audit` trait
- `id()` returns `"code-bare-except"`
- `group()` returns `AuditGroup::CodeQuality`
- `layer()` returns `AuditLayer::Source`
- `applicable()` audits `project.language == Some(Language::Python)`
...
- `src/audits/source/rust/unwrap.rs` — identical structure, different language and pattern
...
- Audit returns Fail with evidence for bare `except:` clauses
- Audit returns Pass for properly typed exception handlers
- Evidence format matches Rust audits: `file:line:column — violation text`

---

- [x] **Unit 2: Implement `p4-sys-exit` Python source audit**

**Goal:** Detect `sys.exit()` calls outside `if __name__ == "__main__"` blocks.

**Requirements:** R1, R2

**Dependencies:** Unit 1 (establishes the Python audit pattern)

**Files:**

- Create: `src/audits/source/python/sys_exit.rs`
- Modify: `src/audits/source/python/mod.rs` — register the audit
...
- Conditional logic: if `sys.exit()` is found, audit whether it's inside an `if __name__ == "__main__"` block
- If found outside the guard, return Fail
- This validates the conditional audit pattern for Python (analogous to `process_exit.rs` for Rust)
- May need to use `find_pattern_matches()` and inspect the match context rather than a single ast-grep pattern

**Patterns to follow:**

- `src/audits/source/rust/process_exit.rs` — same concept, different language
...
- [x] **Unit 3: Implement `p6-no-color-source` Python source audit**
...
- Create: `src/audits/source/python/no_color.rs`
- Modify: `src/audits/source/python/mod.rs` — register the audit
...
- If not found, Warn (not an error — the behavioral audit covers NO_COLOR too)
- This validates cross-language audit parity (same audit exists for Rust)

**Patterns to follow:**

- `src/audits/source/rust/no_color.rs` — same concept, different language and pattern
...
- Returns Warn (not Fail) when missing — behavioral audit is the primary gate
...
**Dependencies:** None (can be done in parallel with Python audits)
...
- `test_broken_fixture`: needs parseable Rust source in `tests/fixtures/broken-rust/`. The fixture intentionally has
  violations — verify ast-grep can parse it (parse errors ≠ audit failures).
- `test_perfect_fixture`: the comment says "requires cargo build" — if behavioral audits need a binary, either build the
...
- Happy path: `test_source_only_fixture` runs source + project audits, no behavioral audits
...
**Dependencies:** Units 1-4 (run validation after all audits and tests are in place)
...
- `agentnative audit ~/dev/bird` — Rust CLI, should produce meaningful results
...
**Goal:** Formally document that v0.1 ships 2-3 starter Python audits, with full coverage in v0.2.

**Requirements:** R5

**Dependencies:** Units 1-3 (know exactly which audits shipped)
...
- Update the "What's Deferred" table: change "Full Python source audits | 5-6 key audits ship in v0.1 | v0.2" to reflect
  that 2-3 starter audits ship in v0.1 and the remaining audits move to v0.2
- Update the constraints section if it references "5-6 key Python audits"
- Keep the table row for "Full Python source audits" as a v0.2 item
...
- **Interaction graph:** Python audits integrate through the same `Audit` trait and
  `all_source_audits(Language::Python)` dispatch. No new infrastructure needed.
- **Error propagation:** ast-grep parse failures for Python source become `AuditStatus::Error` (same as Rust). No new
  error types.
...
- **Unchanged invariants:** All existing Rust audits, behavioral audits, and project audits unchanged. The audit count
...
- **Rust audit pattern:** `src/audits/source/rust/unwrap.rs`
- **ast-grep language support:** `ast-grep-language` crate with `tree-sitter-python` feature
- Related code: `src/source.rs`, `src/audits/source/python/mod.rs`
