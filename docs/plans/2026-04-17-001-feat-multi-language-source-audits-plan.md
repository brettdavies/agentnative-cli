---
title: "feat: Go, Ruby, and TypeScript source audits"
type: feat
status: active
date: 2026-04-17
---

# feat: Go, Ruby, and TypeScript source audits

> **Phase 1 (research spike) shipped 2026-04-17** — Unit 1's ast-grep viability assessment landed at
> [`docs/plans/spikes/2026-04-17-multi-language-source-audits-spike.md`](spikes/2026-04-17-multi-language-source-audits-spike.md)
> (`status: complete`).
>
> **Phase 2 (Units 2–6 — Go / TypeScript / Ruby starter audits + cross-cutting infra + fixtures)** is **deferred to
...
Extend source audits beyond Rust and Python to cover Go, Ruby, and TypeScript. Start with a scoped research spike to
characterize ast-grep support, meta-variable conventions, framework idioms, and code-pattern viability per language,
then ship 3–4 starter audits per language behind the same `Audit` trait architecture already proven by the 16 Rust
audits and 3 Python audits. This is the next increment on the design doc's commitment that source audits "go deeper for
...
audits for both. Behavioral and project audits already run language-agnostically, so any CLI benefits from the tool on
day one. But the design doc's positioning — "deeply useful to Rust/Python/Go/Node developers" — is still half-shipped:
Go and Node projects are *detected* (`detect_language` reads `go.mod` and `package.json`) but get zero source audits,
...
Because the languages differ enough that the wrong assumption would cost an entire rewrite of an audit module, the user
...
- R1. A spike document that characterizes ast-grep support, meta-variable conventions, viable pattern shapes, starter
  audit candidates, and binary-size/compile-time cost per language (Go, Ruby, TypeScript) before any audit code is
  written.
- R2. Go source audits: 3–4 starter audits implemented and covered by unit tests, registered via
  `all_source_audits(Language::Go)`.
- R3. TypeScript source audits: 3–4 starter audits implemented and covered by unit tests, registered under whichever
  Language variant the spike recommends (new `Language::TypeScript` or reuse of `Language::Node`).
- R4. Ruby source audits: 3–4 starter audits implemented and covered by unit tests, including a new `Language::Ruby`
...
- R5. Integration tests: one new fixture per language exercising at least one passing and one failing audit path.
...
- R7. All existing Rust and Python audits continue to pass dogfooding (`anc audit .`) with no regressions in scorecard
  output, binary size beyond the new grammar cost, or CI matrix.

## Scope Boundaries

- Not implementing the full 7-principle audit set per language at launch — 3–4 starter audits each, with the remainder
  deferred to a follow-up iteration once the pipeline is proven (same approach that worked for Python in the v0.1 plan).
- Not adding `--fix`/rewrite support for the new audits — the ast-grep architecture supports it, but it is still
  deferred to v0.2 for every language.
- Not extending behavioral audits — they are already language-agnostic and need nothing here.
- Not adding JavaScript-specific source audits. If the spike recommends keeping `Language::Node` as a JS-only variant
  distinct from a new `Language::TypeScript`, JS source audits are deferred.
- Not adding Tsx-specific AST walking quirks beyond what `.tsx` fixtures require — frontend-framework audits (React
...
- Full principle coverage (P1–P7) per language: each language ships 3–4 starter audits here; the remainder is tracked in
  a follow-up plan once starter coverage lands.
- JavaScript-specific source audits (separate from TypeScript): deferred until the Node/TypeScript variant decision is
  implemented.
...
- `src/audits/source/mod.rs` — `all_source_audits(Language)` router; today only Rust and Python have populated
  sub-modules.
...
- `src/audits/source/rust/process_exit.rs` — multi-pattern audit with file-scope exemption (main.rs); template for TS/Go
  entry-point exemptions.
- `src/audits/source/rust/mod.rs` and `src/audits/source/python/mod.rs` — registration pattern (`all_<lang>_audits()`
  returns `Vec<Box<dyn Audit>>`).
- `src/audit.rs` — `Audit` trait: `id()`, `group()`, `layer()`, `applicable()`, `run()`. `run()` is the sole
  `AuditResult` constructor (see CLAUDE.md's Source Audit Convention).
...
- `docs/plans/2026-04-02-004-feat-python-audits-validation-coverage-plan.md` — the successful v0.1 Python expansion.
  Pattern: start with 2–3 starter audits per new language, prove the pipeline end-to-end (parse → match → evidence), add
...
- `run()` is the sole `AuditResult` constructor; `audit_x()` helper returns `AuditStatus`.
- For cross-language pattern helpers, extend `source::has_pattern_in()` / `find_pattern_matches_in()` /
  `has_string_literal_in()` with new `Language` variants — no private per-language helpers in individual audit files.
...
- Go CLI framework landscape: standard library `flag`/`os`, `spf13/cobra`, `urfave/cli`. Idioms include explicit error
  ...
  audits. The `os.Getenv("NO_COLOR")` convention is a direct analog to the Rust/Python NO_COLOR audit.
...
- **TypeScript is a distinct `Language::TypeScript` variant, not a sub-mode of `Language::Node`.** Rationale: (1) the
  audit set diverges — TypeScript has structural audits (type annotations, interface shapes) that do not apply to JS;
...
- **Grammar cost accepted, measured.** Adding three tree-sitter grammars materially grows the `anc` binary. The spike
  ...
  reconsider scope.
- **No private per-language helpers in audit files.** All Go/TS/Ruby audits must use `src/source.rs` helpers extended
  with their `Language` variant. This mirrors the Python convention codified in CLAUDE.md and prevents drift.
- **Starter audit selection criteria.** Each language ships 3–4 audits chosen to (a) prove the pipeline (one simple
  `Pattern` audit, one AST-walking audit), (b) map to at least two distinct principles (P1–P7), and (c) exercise
...
- **Ordering: Go → TypeScript → Ruby.** Go reuses an existing Language variant, uses the familiar `.go` single
  extension, and has the closest idiom overlap with Rust audits already written. TypeScript is second — adds
  multi-extension walk and new Language variant but stays in `$`-style meta-var territory. Ruby is last — introduces the
  new Language variant, meta-var character, and the highest idiomatic distance from the existing audit set, so it
...
- **Does this plan change the behavioral audit set?** No. Behavioral audits run the binary and are language-agnostic;
  they need no changes. Only the source-audit layer grows.
...
- **Exact starter audit list per language.** Candidate sets appear in Units 3–5 but must be validated by the spike (Unit

1) — a pattern may fail to match the AST shape we expect, or an audit may turn out to be redundant with an existing
   behavioral audit. The spike is the gate.
...
- **TypeScript entry-point heuristic.** For a `process::exit`-style audit in TS, what counts as the entry point? Options
...
- **Whether to include a `.rake` extension in Ruby's walk.** Decide during Unit 2 based on whether starter audits apply
  to Rakefiles — likely not.
- **Node-kind-based Tsx handling.** If any TS audit needs to run against `.tsx` files specifically (React components),
  decide whether to dispatch on `Tsx` vs `TypeScript` per-file or uniformly parse all `.ts*` as `TypeScript`. The
  starter audits are unlikely to need this distinction; revisit if an audit becomes Tsx-sensitive.
...
source.rs dispatch (has_pattern_in, find_pattern_matches_in, has_string_literal_in)
  match lang {
    ...
    Node       => placeholder — empty results until JS audits land,
  }

Per-language audit module layout (mirrors src/audits/source/python/)
  src/audits/source/go/mod.rs            -> all_go_audits()
  src/audits/source/go/<audit>.rs        -> Audit impl + audit_x() helper + tests
  src/audits/source/typescript/mod.rs    -> all_typescript_audits()
  src/audits/source/typescript/<audit>.rs
  src/audits/source/ruby/mod.rs          -> all_ruby_audits()
  src/audits/source/ruby/<audit>.rs

all_source_audits(Language) router extends to include Go, TypeScript, Ruby branches.
```

## Implementation Units

- [x] **Unit 1: Research spike — ast-grep viability and starter audit candidates** — completed 2026-04-17, report at
  `docs/plans/spikes/2026-04-17-multi-language-source-audits-spike.md`
...
starter audits with their proposed ast-grep pattern strings or AST-walk node kinds, the approximate binary-size delta
...
- Create: `docs/plans/spikes/2026-04-17-multi-language-source-audits-spike.md`
...
- Enumerate 4–6 candidate starter audits per language, mapping each to an agent-readiness principle (P1–P7 or
  `code-quality`) and citing an existing Rust/Python audit as the template.
...
- The original `ast-grep-core v0.42.0` spike noted in `CLAUDE.md` (three PoC audits + 18 tests) is the precedent for
...
- Report documents, for each of Go, TypeScript, and Ruby: feature flag, exposed type, meta-var char, 4–6 starter audit
...
- Modify: `src/audits/source/mod.rs` (all_source_audits router — add TypeScript, Ruby branches; Go branch already exists
  but currently returns `vec![]`, will be wired to `go::all_go_audits()` in Unit 3).
...
- Audit CI impact: the stripped binary will grow; update `scripts/hooks/pre-push` and CI matrix expectations only if the
  spike recorded a binary-size cliff that warrants a new audit.
...
- Python-audit file-name → extension convention.
...
- `anc audit .` (dogfooding) returns the same scorecard it did before the change — no Go/TS/Ruby audits run on this Rust
  repo, so the scorecard is unchanged.

---

- [ ] **Unit 3: Go source audits — starter set**

**Goal:** Implement 3–4 Go starter audits in `src/audits/source/go/`, register them via `all_go_audits()`, and wire that
into `all_source_audits(Language::Go)`.

**Requirements:** R2.

**Dependencies:** Unit 1 (pattern and audit-list are spike outputs), Unit 2 (Language enum and dispatch must be in
place).

**Files:**

- Create: `src/audits/source/go/mod.rs` (re-exports plus `all_go_audits()`).
- Create: `src/audits/source/go/<audit>.rs` for each starter audit. Starter list confirmed by Unit 1's spike report
  (section "Per-grammar findings: Go → Starter audit candidates"):
...
- Modify: `src/audits/source/mod.rs` — wire `all_source_audits(Language::Go)` to `go::all_go_audits()`.
- Test: inline `#[cfg(test)]` module per audit file (matches Python convention: `audit_x()` helper tested directly,
  `run()` tested with a temporary-directory fixture).

**Approach:**

- Each audit follows the Python file layout exactly (see `src/audits/source/python/bare_except.rs` for AST-walking shape
  and `src/audits/source/python/sys_exit.rs` for guard-scope shape). The `Audit` trait implementation is the sole
  `AuditResult` constructor; `audit_x()` helpers return `AuditStatus`.
- Use `src/source.rs` helpers (extended in Unit 2) — do not write private per-language helpers.
- Include explanatory module doc comments naming the principle and the analogous Rust/Python audit.

**Execution note:** Implement test-first — write the inline unit tests around `audit_x()` before the `run()` wiring so
spike pattern decisions are validated empirically per audit.

**Patterns to follow:**

- `src/audits/source/rust/unwrap.rs` — simplest pattern audit (pattern string + helper-call + Pass/Fail).
- `src/audits/source/rust/process_exit.rs` — multi-pattern + entry-point exemption.
- `src/audits/source/python/no_color.rs` — pattern-list + string-literal fallback.

**Test scenarios:**

- Happy path: Each audit's Pass case — a Go source string that should not trigger — returns `AuditStatus::Pass`.
- Happy path: Each audit's Fail/Warn case — a Go source string that should trigger — returns the right status with
...
- Edge case: Each audit called with `""` (empty source) returns `Pass` without panicking.
- Error path: Each audit with a syntactically invalid Go source does not panic — either ast-grep parses best-effort or
  the helper returns `Pass` / empty matches (must match existing Rust/Python behavior).
- Integration: `Audit::applicable` returns true for a project whose `Project::language` is `Some(Language::Go)` and
  false for any other variant.
- Integration: `Audit::run` against a temp-dir project with a Go file in `src/` aggregates evidence across multiple
  files and returns the combined `AuditStatus`.

**Verification:**

- `cargo test --locked` passes all new Go tests plus every existing test.
- `anc audit` against a small Go fixture repo (see Unit 6) produces the expected scorecard rows for the new audits.
- Dogfooding (`anc audit .`) remains green — Go audits are not applicable to this Rust repo.

---

- [ ] **Unit 4: TypeScript source audits — starter set**

**Goal:** Implement 3–4 TypeScript starter audits in `src/audits/source/typescript/`, register them via
`all_typescript_audits()`, and wire into `all_source_audits(Language::TypeScript)`.
...
- Create: `src/audits/source/typescript/mod.rs` and per-audit files. Canonical candidates (final list from spike):
...
- Dispatch via `ast-grep-language::TypeScript`. If an audit must run against both `.ts` and `.tsx`, parse each file
  individually using the extension to pick `TypeScript` vs `Tsx` — only add this branching if the spike says a starter
  audit needs it. For the starter list, uniform `TypeScript` parsing is likely sufficient.
- Entry-point heuristic: recommended algorithm (to be confirmed in spike) — read `package.json`'s `bin` object; if
  ...
  the file referenced by `main`. Document the heuristic's limitations in the audit's module doc comment.

**Execution note:** Characterization-first for `process_exit_outside_entry` — start with a passing and a failing fixture
so the entry-point heuristic is validated against real code before the audit ships.

**Patterns to follow:**

- `src/audits/source/rust/process_exit.rs` — file-path exemption for main.
- `src/audits/source/rust/naked_println.rs` — output-module heuristic.
- `src/audits/source/python/no_color.rs` — pattern-list + string-literal fallback.

**Test scenarios:**

- Happy path: Each audit's Pass and Fail cases produce expected statuses and evidence.
- Edge case: `.tsx` parsing — a React component file does not cause a panic or false positive for any starter audit (the
  starter audits target backend-flavored code).
...
- Integration: `Audit::applicable` returns true only for `Language::TypeScript`; Node, Rust, Python, Go, Ruby all return
  false.
- Integration: `Audit::run` aggregates evidence across `.ts` and `.tsx` files under the project.
...
- [ ] **Unit 5: Ruby source audits — starter set**

**Goal:** Implement 3–4 Ruby starter audits in `src/audits/source/ruby/`, register via `all_ruby_audits()`, wire into
`all_source_audits(Language::Ruby)`, and validate `µ` meta-var usage end-to-end.
...
- Create: `src/audits/source/ruby/mod.rs` and per-audit files. Canonical candidates (final list from spike):
...
- Patterns for Ruby use the `µ` meta-variable character (confirmed by spike Unit 1). Document this prominently in
  `src/audits/source/ruby/mod.rs` as a module-level doc comment so future contributors don't write `$VAR` patterns by
  mistake.
- AST-walking for `bare_rescue` follows the Python `bare_except` pattern exactly — walk the tree, match on the `rescue`
  node kind, audit whether the node has an exception-class child.
...
- `src/audits/source/python/bare_except.rs` — AST-walking structural audit.
- `src/audits/source/python/sys_exit.rs` — entry-point idiom detection via header-text pattern match across several
  canonical forms.
- `src/audits/source/python/no_color.rs` — pattern-list + string-literal fallback.
...
- Integration: `Audit::run` aggregates evidence across `.rb` files.
...
**Goal:** Add one fixture directory per new language exercising at least one passing and one failing audit; add or
...
- Create: `tests/fixtures/broken-go/` with `go.mod` plus a `.go` file containing at least one violation per Go starter
  audit.
- Create: `tests/fixtures/perfect-go/` with a clean Go project that passes every Go starter audit.
- Create: `tests/fixtures/broken-typescript/` with `tsconfig.json` + `package.json` plus `.ts` and `.tsx` files
  containing at least one violation per TS starter audit.
...
- Modify: `README.md` — update the Supported Languages section (list Go, TypeScript, Ruby with their starter audit
  counts).
- Modify: `CLAUDE.md` — update the "Architecture" / "Source Audit Convention" sections to reference the new per-language
...
- Integration tests follow the existing assertion style (audit the aggregated scorecard includes expected rows with
...
- CLAUDE.md update: append a note to the "Conventions" or "Source Audit Convention" section documenting that Go and Ruby
...
- Existing CLAUDE.md "Source Audit Convention" and "Conventions" sections.

**Test scenarios:**

- Integration: `anc audit tests/fixtures/broken-go` exits non-zero and the scorecard shows each Go starter audit with a
  failure/warning as appropriate.
- Integration: `anc audit tests/fixtures/perfect-go` exits zero and the scorecard shows each Go starter audit as Pass.
...
- Integration: Scorecard output format remains stable for existing Rust/Python fixtures (regression guard — Unit 6 must
  not change existing snapshots beyond adding rows for newly registered audits).
- Edge case: A TypeScript fixture containing both `.ts` and `.tsx` files produces evidence from both when an audit
  matches in both.
...
- CLAUDE.md changes merged without accidentally breaking the existing `Source Audit Convention` instructions the team
  relies on.
...
- **API surface parity:** The `Audit` trait is unchanged. The `Language` enum gains two variants (`TypeScript`, `Ruby`);
...
- **Integration coverage:** Unit 6's fixtures cover the end-to-end path (`Project::discover` → `all_source_audits` →
  scorecard) per language, which is the only coverage surface that unit tests against in-memory source strings cannot
  prove.
- **Unchanged invariants:** Existing Rust and Python audits must continue to run on Rust/Python projects with identical
  scorecard output. Behavioral audits are unchanged. `anc`'s CLI surface (commands, flags) is unchanged — this plan adds
  source audits only, no new CLI entry points, no new flags. The dogfooding rules in `CLAUDE.md` ("bare invocation
...
| Go top-level pattern-parse failure for selector-expression calls (e.g. `os.Exit($CODE)` parses as `ERROR > type_conversion_expression` at the top level, not as a `call_expression`). Surfaced by spike (2026-04-17).                                                                                  | Unit 3 uses AST walking for selector-call audits (`os.*`, `log.*`, `fmt.*`) and reserves `Pattern::try_new` for bare builtin calls (`panic`, `print`). Every Go starter audit ships with a Fail test case that would surface a pattern-parse regression immediately. |
| Ruby uppercase-global literal (`$PROGRAM_NAME`) cannot appear in a `Pattern` string — the expando-char rewriter turns it into a meta-var. Surfaced by spike (2026-04-17).                                                                                                                              | Unit 5's entry-point audit uses AST walking + header-text inspection (same shape as Python's `sys_exit::is_main_guard`) rather than a literal `Pattern`. The `$0` digit global is unaffected and can appear in patterns as-is.                                       |
...
| Starter audit selection turns out to be redundant with existing behavioral audits (e.g., a TS `process_exit_outside_entry` audit duplicates what `--help exits 0` already proves).                                                                                                                     | Spike criteria include "maps to at least two distinct principles" and "proves cross-language helper semantics" — this forces diversity. Any audit that only duplicates an existing behavioral audit is dropped in Unit 1.                                            |
| Meta-var character confusion causes silent pattern mismatches (no panic, just never matches).                                                                                                                                                                                                          | Every per-language audit's Fail test case is a concrete source string known to trigger — if the pattern is wrong the test fails, surfacing the issue at Unit 3/4/5 commit time rather than in production.                                                            |
| Adding three grammars meaningfully slows compile times for every contributor, even those working only on Rust audits.                                                                                                                                                                                  | Accepted — this is core product work, not optional tooling. If the slowdown is painful, a follow-up could split grammars behind Cargo features; not in scope here.                                                                                                   |

## Documentation / Operational Notes

- The spike report itself (Unit 1) is a permanent doc at
  `docs/plans/spikes/2026-04-17-multi-language-source-audits-spike.md` and is referenced from this plan; do not delete
...
- **Use ast-grep's YAML rule files instead of Rust code for pattern definitions.** Considered. Upside: easier to edit
  ...
  requires bundling YAML as string assets, and diverges from the existing Rust/Python audit architecture. Rejected for
  this plan; revisit as a distinct architectural plan if external contributor velocity becomes a bottleneck.

## Success Metrics

- 10–12 new source audits (3–4 per language × 3 languages) land, each with ≥80% test coverage on its `audit_x()` helper.
...
- `anc` binary size growth falls within the cap documented in the spike (strictly below 15 MB total delta across all
  three grammars).
- `anc audit tests/fixtures/broken-<lang>/` produces non-zero exit and scorecard rows that match the audits defined for
...
- Unit 2 lands as a single code PR. Purpose: make `Language::TypeScript` and `Language::Ruby` real, dispatch the new
  grammars, and keep every existing test green. No new audits yet.

### Phase 3 — Per-language audits
...
- Design doc: `~/.gstack/projects/brettdavies-agentnative/brett-main-design-20260327-214808.md` (the design doc
  ...
  plus Ruby).
- Prior plan: `docs/plans/2026-04-02-004-feat-python-audits-validation-coverage-plan.md` (the successful Python
  starter-audit rollout this plan mirrors).
- Existing code: `src/source.rs`, `src/project.rs`, `src/audits/source/python/`, `src/audits/source/rust/`,
  `src/audit.rs`, `src/audits/source/mod.rs`.
...
- Conventions: `CLAUDE.md` (Source Audit Convention, Dogfooding Safety, CI and Quality sections).
