...
- R1. `anc audit --output json` emits `schema_version: "0.4"` with four new top-level objects: `tool`, `anc`, `run`,
...
- R4. `run` carries `{ invocation, started_at, duration_ms, platform }`. `invocation` is the full argv as the user typed
  ...
  wall-clock milliseconds from the start of `Commands::Audit` to scorecard emission. `platform` is `{ os, arch }`
...
- `src/argv.rs` — `inject_default_subcommand` is where `anc .` becomes `anc audit .`. The `run.invocation` field MUST
...
- `src/cli.rs` — `Commands::Audit { path, command, ... }` carries the resolved fields needed to populate `target`.
- `src/main.rs` — the entry point that owns the `Commands::Audit` arm and currently calls `format_json`. The new
...
- **Capture invocation pre-injection.** The recorded command reflects user intent. A user who typed `anc .` should see
  `"anc ."` in the scorecard, not `"anc audit ."` — the latter is a fact about anc's internals, not the user's command.
...
- **Timestamp library choice.** Neither `chrono` nor `time` is in `Cargo.lock` today — both are net-new direct deps.
  ...
  1-line call.
- **`run.duration_ms` start point** — fully resolved by R4 (start of `Commands::Audit` arm to scorecard emission). No
...
- A manual `anc audit . --output json | jaq .` shows the four new objects in the expected shape.
...
- Pass the formatted invocation through to `build_scorecard` via the `Commands::Audit` arm.
...
- A run of `anc .` (default-subcommand injection path) records `run.invocation: "anc ."`, **not** `"anc audit ."`.
...
- Modify: `src/main.rs` (the `Commands::Audit` arm)
...
- Capture `std::time::Instant::now()` at the start of `Commands::Audit`.
...
- U5. **Populate `tool` and `target` from the resolved `Commands::Audit` args**

**Goal:** Translate the three `anc audit` modes (project / binary / command) into structured `target` + `tool` metadata,
...
- Happy path: `anc audit --command echo` produces `target.kind = "command"`, `target.command = "echo"`, `tool.name =
  "echo"`, `tool.version` = whatever `echo --version` first line trims to (system-dependent — assert shape, not exact
  text).
- Happy path: `anc audit ./fixture-project` produces `target.kind = "project"`, `target.path` ends with
  `fixture-project`.
...
- Edge case (security): `anc audit --command anc` (or `--command <path-to-current-anc-exe>`) → self-spawn guard
...
- Integration: `anc audit . --output json | jaq '.target.kind'` returns `"project"` for the project's own root.

**Verification:**

- `cargo test` is green. Manual end-to-end: `anc audit --command rg --output json | jaq .tool.version` returns a
...
- **Integration coverage:** U6's integration test exercises the three `anc audit` modes end-to-end. Unit-only coverage
