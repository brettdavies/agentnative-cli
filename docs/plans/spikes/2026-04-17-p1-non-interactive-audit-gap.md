...
- **But the audit is narrow.** It only probes one thing: *does the binary complete under `stdin = /dev/null` within 5s?*
...
- **Concrete gap**: three of P1's four MUST bullets are unverified by any audit today (behavioral OR source). The only
  source-layer audit that mentions P1 (`p1-non-interactive-source`) is applicable only to Rust projects with a
...
- **Recommendation**: the bare-invocation timeout probe is doing what it can. Strengthening P1 coverage is a
  ...
  flag-existence audit, an env-var-hint scan, and an auth-subcommand headless-flag scan) before trying harder things
  like PTY probing.
...
The `p1-non-interactive` behavioral audit is implemented at `src/audits/behavioral/non_interactive.rs`. Its probe is a
...
The audit's classification logic (excerpted verbatim from `non_interactive.rs`):
...
receive a signal. Exit code is **not consulted** for this audit's verdict.
...
Read against the audit's classification:
...
→ `Pass`. Exit code 1 is irrelevant to this audit.
...
exactly that, under one condition: the non-interactive path must be fully functional. The audit is structured to detect
...
- Tool with TUI + working non-TTY path (claude): agent runs it with piped stdin, gets error message, proceeds. No
  deadlock.
- Tool with TUI + TUI-even-under-non-TTY (the P1 failure mode the audit is looking for): agent calls it, process hangs
...
true. It does not say "this tool satisfies every MUST in P1" — no single audit does today.
...
3. **Environment-dependent interactivity.** A tool that reads `$TERM`, `$CI`, or `$DEBIAN_FRONTEND` to decide whether to
   prompt. The audit could set these to non-interactive values but currently does not.
...
audit. The source-layer analog (below) is where structural coverage should grow.

## The source-layer audit today
...
Against the claude probe, this audit is **not applicable** (the input is a binary file, not a Rust directory project).
Against a Python, Go, Ruby, TypeScript, or Node project, this audit is **not applicable** either (Rust-only). Against a
Rust project that uses a prompt library not on the list (e.g., `requestty`, `demand`, `promkit`, `cliclack`), this audit
would return Pass.

This is a narrow safety net, not a P1 compliance test. The principle doc's footer claim that P1 is "Measured by audit
IDs `p1-non-interactive` (behavioral) and `p1-non-interactive-source` (source)" is technically accurate but under-sells
how limited both audits are.

## Recommendations

These are scoped as a standalone follow-up plan; not in scope for this spike. Roughly in increasing order of cost:

1. **Flag-existence scan in `--help` output** (new behavioral audit, or extend `p1-non-interactive`). Look for at least
...
2. **Env-var hint scan in `--help` output** (new behavioral audit, `p1-env-var-bindings`). Parse `--help` for per-flag
...
3. **Auth-subcommand headless-flag scan** (new behavioral audit, `p1-no-browser`). If `<binary> --help` lists an auth
...
If a follow-up ships, frame it as "P1 audit depth, not breadth." The MUST list in the principle has four bullets;
today's audits cover zero of them structurally (the behavioral probe verifies an implication of the SHOULD, not a MUST).
Four additive audits (one per MUST bullet where feasible) would close that gap without disturbing the existing probe.
...
env-var binding on its --quiet flag"). Keep them as separate audits so the scorecard remains legible.

## Addendum: is the "agent = non-TTY" assumption even true?

The audit's design implicitly assumes that when an agent calls a CLI, stdin is a pipe or `/dev/null`. That is true for a
...
- **`expect` / Ansible-style automation.** Some shipping automation wraps CLIs with `expect`, `unbuffer`, or `script`
  specifically so that tools that audit `isatty()` behave as if a human is present.
...
that adds a TTY probe as a new audit (e.g., `p1-non-interactive-tty`) rather than modifying the existing
`p1-non-interactive`. Two orthogonal audits with two orthogonal signals keep the scorecard legible even when one fails
...
interactive UX when a TTY is detected. That is the ground truth the audit mirrors.
...
framing the project wants will produce an audit that flags tools the principle explicitly permits, or permits tools the
project's audience expects to fail. Resolve the framing first; let that drive the probe design.

## Sources

- `src/audits/behavioral/non_interactive.rs` — current behavioral probe; the source of the Pass verdict under
  investigation.
