...
- [`scripts/sync-prose-tooling.sh`](sync-prose-tooling.sh): header comment covers `--check` drift mode and the manifest.
  `scripts/prose-check.sh` is consumer-owned (un-vendored 2026-05-13); its CONSUMER-OWNED header explains why and what
  coordination universal pipeline changes now require.
