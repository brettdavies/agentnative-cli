...
flowchart LR
    ...
    CLI -->|site's regen-scorecards.sh<br/>(anc check ... --output json)| SITE_SCORE
...
| `brettdavies/agentnative` (spec) @ latest `v*` tag           | `scripts/sync-prose-tooling.sh` (manual; `--check` mode for drift)        | `BRAND.md` + `styles/brand/` (rule pack) + `styles/config/vocabularies/brand/` (vocab) + `scripts/prose-check.sh` (orchestrator) + `scripts/test-prose-check.mjs` + `scripts/generate-pack-readme.mjs`. Per-consumer config (`.vale.ini`, `styles/config/vocabularies/cli/`) authored locally; not vendored. | Rerun after the spec ships a tag touching any path in the manifest. Idempotent at a fixed tag. **CLI-LOCAL DIVERGENCE:** `scripts/prose-check.sh` carries a marked patch block adding CLI-specific path exclusions (`src/principles/spec/`, `docs/ideation/`, `tests/fixtures/`, `coverage-matrix.md`). Re-apply post-sync until upstream lands `--exclude PATTERN` (tracked at [agentnative-spec#010](https://github.com/brettdavies/agentnative/blob/dev/.context/compound-engineering/todos/010-pending-p0-prose-check-consumer-exclusion-config.md)). | `--check` mode compares each vendored file byte-for-byte against the upstream tag. `scripts/prose-check.sh` is intentionally excluded from `--check` because of the inline CLI-LOCAL DIVERGENCE patch; rely on `git diff scripts/prose-check.sh` post-sync to surface what got reset. |
...
3. **Manual** — if the registry shape or covers map changed, run `anc generate coverage-matrix` and commit
...
- [`scripts/sync-prose-tooling.sh`](sync-prose-tooling.sh) — header comment covers `--check` drift mode and the
  CLI-LOCAL DIVERGENCE contract for `scripts/prose-check.sh`.
