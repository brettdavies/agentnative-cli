...
flowchart LR
    subgraph Inbound["Inbound — data INTO this repo"]
        ...
        SITE_IN["agentnative-site<br/>src/data/skill/skill.json"]
        MAIN["self: main branch<br/>(Cargo.toml, Cargo.lock, CHANGELOG,<br/>release-prep edits)"]
    end
...
| -------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `brettdavies/agentnative` (spec); default latest `v*` tag, or any branch/tag/SHA via `--ref` | `scripts/sync-spec.sh` (manual; `gh api`-based, falls back to `$SPEC_ROOT` when offline; `--ref <branch\|tag\|SHA>` or `SPEC_REF` env var vendors any explicit ref for cross-repo coordination of in-flight spec work) | `principles/p*-*.md` + `principles/scoring.md` + top-level `VERSION` + `CHANGELOG.md` → `src/principles/spec/`                                                                                                                                                                                                                                                                                                                            | Rerun after every new `agentnative-spec` `v*` tag. The intended trigger is a `repository_dispatch` from the spec's publish workflow; until that exists, manual. Cross-repo coordination: rerun with `--ref dev` (or a SHA) to consume in-flight spec work before a release cuts; the script prints the resolved short SHA for the consumer PR body. | `build.rs` is *intentionally loud*: it fails on missing `VERSION`, missing `principles/` dir, parse errors, duplicate IDs, or missing fields. `cargo test` (`integration::*` + `dangling_cover_ids`) catches `covers()` IDs that drift from the vendored registry.                                                     |
| `brettdavies/agentnative-site` `src/data/skill/skill.json` @ `dev`                           | `scripts/sync-skill-fixture.sh` (manual; `--check` in CI)                                                                                                                                                              | Skill bundle manifest (install map / hosts) → `src/skill_install/skill.json`                                                                                                                                                                                                                                                                                                                                                              | Rerun whenever the site changes `src/data/skill/skill.json`. Pre-release checklist in `RELEASES.md` step 6 captures this for every release.                                                                                                                                                                                                         | `.github/workflows/skill-fixture-drift.yml` runs `sync-skill-fixture.sh --check` on every PR + push to main/dev. Within a build, the fixture and the Rust codegen map cannot drift relative to each other: both are single-sourced from the committed JSON.                                                            |
| this repo's own `main` branch (release artifacts)                                            | `scripts/sync-dev-after-release.sh vX.Y.Z` (manual; idempotent; opens a PR against `dev`)                                                                                                                              | `Cargo.toml` `[package].version` (in-place line edit), the workspace entries in `Cargo.lock` (refreshed offline), `CHANGELOG.md` (verbatim from `origin/main`), and every path `main` changed since the previous release tag that `dev` left alone (release-prep); paths both branches changed are listed and withheld unless named with `--only PATH` or taken with `--include-contested` → `dev` via a `chore/sync-dev-after-vX.Y.Z` PR | Run AFTER (1) `release/v*` → `main` PR merges, (2) `git tag vX.Y.Z` pushed, (3) `finalize-release.yml` flips the GitHub Release to `published`.                                                                                                                                                                                                     | `scripts/release/postflight.sh backport` looks for the merged PR. Pre-flight checks in the script: working tree clean, tag exists locally, tag is reachable from `origin/main`, GitHub Release is not a draft; re-run on an in-sync `dev` is a no-op. `--dry-run` prints the classification without creating a branch. |
...
2. **Manual.** Propagate any new/changed requirement IDs into `src/audits/*` `Audit::covers()` declarations. The build's
   `dangling_cover_ids` drift detector forces this, so typos surface at `cargo test`, not at render time.
...
1. **Manual pre-release** (`RELEASES.md` step 6). Run `bash scripts/sync-skill-fixture.sh` and review the diff. Catches
   any site-side `skill.json` changes since `dev` was branched. The Rust host map regenerates from the JSON on the next
   `cargo build`, so no manual src edits are needed.
...
4. **Manual post-release.** Run `scripts/sync-dev-after-release.sh vX.Y.Z` and merge the PR it opens against `dev`.
   Backports the release bookkeeping (Cargo.toml version, Cargo.lock's workspace entries, CHANGELOG.md from main) and
   every release-prep edit `main` made since the previous release tag, so future builds from `dev` report the released
   version and the embedded badge URL points at the right slug. Contested paths are listed and withheld (see the table
   above).
5. **Manual on the site.** Run `scripts/regen-scorecards.sh` against the upgraded `anc` (gated by `MIN_ANC_VERSION` =
   `0.1.3`) to refresh per-tool scorecards. Then `scripts/sync-coverage-matrix.sh` if the matrix changed.
...
sequenceDiagram
    ...
    Maintainer->>CLI: scripts/sync-dev-after-release.sh vX.Y.Z (manual; PR to dev)
```
...
- [`scripts/sync-spec.sh`](sync-spec.sh): header comment has detailed usage, the `--ref <branch|tag|SHA>` / `SPEC_REF`
...
the "commit-over-fetch" decision that anchors why `coverage/matrix.json` is a tracked artifact rather than a build-time
fetch. -
