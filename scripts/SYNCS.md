...
| this repo's own `main` branch (release artifacts)                                            | `scripts/sync-dev-after-release.sh vX.Y.Z` (manual; idempotent; opens a PR against `dev`)                                                                                                                              | `Cargo.toml` `[package].version` and the crate's own `Cargo.lock` entry (in-place line edits) + `CHANGELOG.md` (verbatim from `origin/main`) → `dev` via a `chore/sync-dev-after-vX.Y.Z` PR | Run AFTER (1) `release/v*` → `main` PR merges, (2) `git tag vX.Y.Z` pushed, (3) `finalize-release.yml` flips the GitHub Release to `published`.                                                                                                                                                                                                     | `scripts/release/postflight.sh backport` looks for the merged PR. Pre-flight checks in the script: working tree clean, tag exists locally, tag is reachable from `origin/main`, GitHub Release is not a draft; re-run on an in-sync `dev` is a no-op.              |
...
4. **Manual post-release.** Run `scripts/sync-dev-after-release.sh vX.Y.Z` and merge the PR it opens against `dev`.
   Backports the release bookkeeping (Cargo.toml version, the crate's Cargo.lock entry, CHANGELOG.md from main) so
