...
1. **Manual pre-release** (`RELEASES.md` step 7). Run `bash scripts/sync-skill-fixture.sh` and review the diff. Catches
...
4. **Manual post-release.** Run `./scripts/sync-dev-after-release.sh vX.Y.Z` then `git push origin dev`. Backports the
