//! Audit: `--help` advertises environment-variable bindings for flags.
//!
//! Covers: `p1-must-env-var`. Source-verified coverage already exists via
//! `p1-env-flags-source`; this audit adds the behavioral layer by inspecting
...
pub struct EnvHintsAudit;

impl Audit for EnvHintsAudit {
    fn id(&self) -> &str {
        "p1-env-hints"
    }
    ...
    fn run(&self, project: &Project) -> anyhow::Result<AuditResult> {
        let status = match project.help_output() {
            ...
            Some(help) => audit_env_hints(help.flags().len(), help.env_hints().len()),
        };
...
/// the `AuditStatus` that summarizes them.
fn audit_env_hints(flag_count: usize, env_hint_count: usize) -> AuditStatus {
    if flag_count == 0 {
        return AuditStatus::Skip("target exposes no flags in --help".into());
    }
    if env_hint_count > 0 {
        AuditStatus::Pass
    } else {
...
"#;
    ...
    fn happy_path_env_hint_present() {
        ...
        let status = audit_env_hints(help.flags().len(), help.env_hints().len());
    ...
    fn unit_core_returns_pass_with_any_hint() {
        assert_eq!(audit_env_hints(3, 1), AuditStatus::Pass);
        assert_eq!(audit_env_hints(3, 10), AuditStatus::Pass);
    }

    #[test]
    fn unit_core_warns_when_zero_hints() {
        let status = audit_env_hints(5, 0);
