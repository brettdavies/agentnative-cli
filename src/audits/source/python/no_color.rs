        ...
        let status = if found_any {
            ...
            AuditStatus::Warn(
                "No reference to NO_COLOR found in any Python source file. CLIs should respect \
                 ...
                 transparently — verify via the behavioral audit. See https://no-color.org/"
                    .to_string(),
            )
        };
...
pub(crate) fn source_handles_no_color(source: &str) -> bool {
    ...
    // Common explicit access patterns — parse source once, audit all patterns.
    let access_patterns = [
