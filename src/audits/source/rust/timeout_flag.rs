//! Audit: Detect network code without a `--timeout` flag.
//!
...
impl Audit for TimeoutFlagAudit {
    ...
    fn run(&self, project: &Project) -> anyhow::Result<AuditResult> {
        ...
        for (_path, parsed_file) in parsed.iter() {
            match &audit_timeout_flag(&parsed_file.source) {
                AuditStatus::Skip(_) => {
        ...
        let status = if !has_network {
            AuditStatus::Skip("no network code detected".to_string())
...
/// Audit a single source string for network code and timeout flag.
pub(crate) fn audit_timeout_flag(source: &str) -> AuditStatus {
    let has_network = NETWORK_INDICATORS.iter().any(|ind| source.contains(ind));

    if !has_network {
        return AuditStatus::Skip("no network code detected".to_string());
    }

    // Audit for --timeout flag in source
...
"#;
        let status = audit_timeout_flag(source);
