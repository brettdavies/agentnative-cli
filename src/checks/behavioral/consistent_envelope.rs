//! Check: `p2-should-consistent-envelope`.
//!
...
pub struct ConsistentEnvelopeCheck;

impl Check for ConsistentEnvelopeCheck {
    fn id(&self) -> &str {
...
pub(crate) fn check_envelope(success: &RunResult, error: &RunResult) -> CheckStatus {
        ...
        None => {
            return CheckStatus::Skip(
                "error-mode probe (bad flag + `--output json`) produced no parseable JSON; \
                 the stricter `p2-must-json-errors` check covers this case."
                    .into(),
            );
        }
    };
...
mod tests {
    ...
    fn pass_when_envelopes_match() {
        ...
        assert_eq!(check_envelope(&success, &error), CheckStatus::Pass);
    }
    ...
    fn warn_on_drift() {
        ...
        match check_envelope(&success, &error) {
    ...
    fn skip_when_success_not_json() {
        ...
        match check_envelope(&success, &error) {
            CheckStatus::Skip(msg) => assert!(msg.contains("success-mode")),
    ...
    fn skip_when_error_not_json() {
        ...
        match check_envelope(&success, &error) {
            CheckStatus::Skip(msg) => assert!(msg.contains("error-mode")),
            other => panic!("expected Skip, got {other:?}"),
        }
    }
}
