//! Shared bad-invocation probe for the P2/P4 error-battery checks.
//!
//! Five behavioral checks share the same probe shape — run the binary with a
...
//! five checks consistent and means a future spec edit only touches one file.
//!
...
/// Run the binary with [`BAD_FLAG`]. Cached by the runner across checks, so
/// the five error-battery checks share a single child process.
...
///
/// The check is intentionally lenient: any co-occurrence of `--output` and
