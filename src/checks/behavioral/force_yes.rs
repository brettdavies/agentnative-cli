...
//! probe each one's `--help`, and check for the presence of `--force`,
...
use crate::checks::behavioral::destructive_ops::destructive_subcommands;
...
pub struct ForceYesCheck;

impl Check for ForceYesCheck {
    fn id(&self) -> &str {
        "p5-force-yes"
    }
        ...
        let status = match project.help_output() {
            ...
            Some(top_help) => {
                ...
                if destructive.is_empty() {
                    ...
                    check_force_yes(&destructive, &subhelp)
                }
            }
        };
        Ok(CheckResult {
...
mod tests {
    ...
    fn pass_when_destructive_subcommand_has_force() {
        ...
        assert_eq!(
            check_force_yes(&["delete".to_string()], &subhelp),
            CheckStatus::Pass
        );
    }
    ...
    fn pass_when_destructive_subcommand_has_yes() {
        ...
        assert_eq!(
            check_force_yes(&["purge".to_string()], &subhelp),
            CheckStatus::Pass
        );
    }
    ...
    fn fail_when_destructive_subcommand_missing_flags() {
        ...
        match check_force_yes(&["delete".to_string()], &subhelp) {
    ...
    fn fail_when_destructive_subcommand_help_missing() {
        ...
        match check_force_yes(&["delete".to_string()], &subhelp) {
            CheckStatus::Fail(msg) => assert!(msg.contains("delete")),
            other => panic!("expected Fail, got {other:?}"),
        }
    }
    ...
    fn pass_with_mixed_destructive_set() {
        ...
        assert_eq!(check_force_yes(&destructive, &subhelp), CheckStatus::Pass);
    }
}
