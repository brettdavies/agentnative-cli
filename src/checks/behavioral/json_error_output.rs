//! Check: `p4-should-json-error-output`.
//!
...
pub struct JsonErrorOutputCheck;

impl Check for JsonErrorOutputCheck {
    fn id(&self) -> &str {
...
pub(crate) fn classify_json_error_output(result: &RunResult) -> CheckStatus {
...
mod tests {
    ...
    fn warn_on_plain_text() {
        ...
        match classify_json_error_output(&fake_result(stderr, "")) {
            CheckStatus::Warn(msg) => assert!(msg.contains("not JSON-formatted")),
