...
pub struct StdinInputCheck;

impl Check for StdinInputCheck {
    fn id(&self) -> &str {
        "p6-stdin-input"
    }
    ...
    fn run(&self, project: &Project) -> anyhow::Result<CheckResult> {
        let status = match project.help_output() {
            ...
            Some(help) => check_stdin_input(help),
        };
...
pub(crate) fn check_stdin_input(help: &HelpOutput) -> CheckStatus {
    ...
    if !has_input_verb {
        return CheckStatus::Skip(
            "no input-accepting subcommand detected (process/parse/convert/transform/\
             analyze/validate/format/lint/check); vacuous skip for the conditional \
             SHOULD."
                .into(),
        );
    }
...
mod tests {
    ...
    fn pass_when_input_subcommand_and_stdin_mentioned() {
        ...
        assert_eq!(check_stdin_input(&help), CheckStatus::Pass);
    }
    ...
    fn warn_when_input_subcommand_without_stdin_signal() {
        ...
        match check_stdin_input(&help) {
            CheckStatus::Warn(msg) => assert!(msg.contains("stdin")),
