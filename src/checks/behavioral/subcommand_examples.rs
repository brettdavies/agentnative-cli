//! Check: `p3-must-subcommand-examples`.
//!
...
pub struct SubcommandExamplesCheck;

impl Check for SubcommandExamplesCheck {
    fn id(&self) -> &str {
    ...
    fn run(&self, project: &Project) -> anyhow::Result<CheckResult> {
        let status = match project.help_output() {
            ...
            Some(top_help) if top_help.subcommands().is_empty() => CheckStatus::Skip(
            ...
            Some(top_help) => {
                ...
                let binary_name = project
                    ...
                    .and_then(|s| s.to_str());
                check_subcommand_examples(binary_name, &subhelp)
            }
        };
        Ok(CheckResult {
...
pub(crate) fn check_subcommand_examples(
...
mod tests {
    ...
    fn pass_with_dollar_prompt() {
        ...
        assert_eq!(
            check_subcommand_examples(Some("tool"), &subhelp),
            CheckStatus::Pass
        );
    }
    ...
    fn fail_when_subcommand_missing_example() {
        ...
        match check_subcommand_examples(Some("tool"), &subhelp) {
            CheckStatus::Fail(msg) => {
                assert!(msg.contains("generate"));
                assert!(!msg.contains(" check,"), "check should pass: {msg}");
            }
    ...
    fn skip_when_no_subhelp() {
        ...
        match check_subcommand_examples(Some("tool"), &subhelp) {
            CheckStatus::Skip(msg) => assert!(msg.contains("no subcommands")),
