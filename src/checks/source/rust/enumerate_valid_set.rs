//! Check: `p4-should-enumerate-valid-set`.
//!
...
//! "invalid value" error names every accepted variant. This check verifies the
...
pub struct EnumerateValidSetCheck;

impl Check for EnumerateValidSetCheck {
    fn id(&self) -> &str {
    ...
    fn run(&self, project: &Project) -> anyhow::Result<CheckResult> {
        ...
        for (_path, parsed_file) in parsed.iter() {
            match check_enumerate_valid_set(&parsed_file.source) {
...
pub(crate) fn check_enumerate_valid_set(source: &str) -> EnumerateScan {
...
"#;
        assert_eq!(
            check_enumerate_valid_set(source),
            EnumerateScan::ClosedSetDeclared
        );
    }
...
"#;
        assert_eq!(check_enumerate_valid_set(source), EnumerateScan::NoClap);
    }
    ...
    fn applicable_for_rust() {
        ...
        assert!(EnumerateValidSetCheck.applicable(&project));
    }
