...
//! operations". The check uses a heuristic on parsed file content — presence
...
pub struct SigtermCheck;

impl Check for SigtermCheck {
    fn id(&self) -> &str {
        "p6-sigterm"
    }
    ...
    fn run(&self, project: &Project) -> anyhow::Result<CheckResult> {
        ...
        let status = match (has_long_running, has_handler) {
            (false, _) => CheckStatus::Pass, // vacuous — not long-running
...
async fn serve() {}
"#;
        assert_eq!(check_sigterm(source), CheckStatus::Pass);
    }
